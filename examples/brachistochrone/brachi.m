...
% ===========================
x = traj(ninterv,5,6); % Arguments are ninterv, smoothness, order
y = traj(ninterv,5,6);
tf = traj(1,0,1);
...
Constr = constraint(0,'x',0,'initial') + ... % x(0)
    ...
    constraint(0.0001,'tf',Inf,'initial') + ...   %
    ...
    constraint(-1e-6,'(xd^2 + yd^2) - tf^2*(V0^2 + 2*g*y)',1e-6,'trajectory'); % Dynamics as a path constraint
...
subplot(2,2,2);plot(refinedTimeGrid,-th); xlabel('Time'); ylabel('\theta'); hold on;
